...
void switchstmtVerification(nAryTreeNode* switchNode, SymbolTable* currentSymbolTablePtr){
    nAryTreeNode* idNode = switchNode->leftMostChild;
    type* t = searchHashTablePtr(currentSymbolTablePtr->table, idNode->token.id);
    // printf("%s %d\n",idNode->token.id,t->name);
    if(t->name != TK_INT){
        printf(RED"error: line %d: "RESET, idNode->token.line_no);
        printf("Switch variable \"%s\" not an integer\n", idNode->token.id);
    }
}

void offsetstmtVerification(nAryTreeNode* offsetNode, SymbolTable* currentSymbolTablePtr){
  nAryTreeNode* idNode = offsetNode->leftMostChild->brother;
...
void constructSemanticsVerification(nAryTreeNode *root)
{
  ...
  if (root->sym.isTerminal == false)
  {
    ...
    else if(root->sym.nt == offsetstmt){
      offsetstmtVerification(root, root->scopeSymTab);
    }
    else if(root->sym.nt == switchstmt){
        switchstmtVerification(root, root->scopeSymTab);
    }
...
void countVars(nAryTreeNode* root){
    if(!root){
      return;
    }
    // printf("Q2");
    if(!(root->sym.isTerminal) && root->sym.nt == iterativestmt){
        countUnassignedVariables(root);
    }
    nAryTreeNode* N = root->leftMostChild;
    while(N){
        countVars(N);
        N = N->brother;
    }
...
void countUnassignedVariables(nAryTreeNode* whileNode){
    loopVars* lvs = whileNode->extraArguments;
    loopVar* lv = lvs->firstVar;
    int count = 0;
    while(lv != NULL){
    ...
    printf("At while loop starting at line number %d, %d number of variables are unassigned\n", whileNode->token.line_no, count);
}
